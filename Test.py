...
st.markdown("""
    ...
    .welcome-container {
        ...
        min-height: 90vh; /* 占屏幕90%高度 */
        text-align: center;
        padding: 20px;
        box-sizing: border-box;
    }
    .welcome-title {
        font-size: clamp(2rem, 5vw, 3rem); /* 响应式字体：2rem~3rem，随屏幕宽度变化 */
        margin-bottom: 2rem;
        color: #2c3e50;
    }
    .access-count {
...
def plot_3d_map(school_data, display_options=None):
    # 核心修改：图表尺寸改为响应式（基于屏幕宽度）
    # 获取屏幕宽度（通过CSS变量，简化版用相对尺寸）
    fig_width = plt.gcf().get_figwidth()
    fig_height = fig_width * 0.7  # 宽高比 10:7，自适应
...
def main():
    ...
    if st.session_state['page'] == 'welcome':
        ...
        st.image("welcome_image.jpg", use_column_width=True)  # 图片自适应列宽
    ...
    # 主页面（自适应）
        ...
        # 页脚改为自适应（取消固定定位）
        st.markdown('<div class="custom-footer">Created By DANIEL HAN</div>', unsafe_allow_html=True)
