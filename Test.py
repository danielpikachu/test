    ...
    for level in school_data['buildingA']['levels']:
        ...
        for corr_idx, corridor in enumerate(level['corridors']):
            ...
            for p_idx, (px, py, pz) in enumerate(points):
                ...
                ax.text(px, py, pz, f'C{corr_idx}-{p_idx}', color='darkblue', fontsize=8)
...
def build_navigation_graph(school_data):
    ...
    for level in school_data['buildingA']['levels']:
        ...
        # 2. 不同走廊间的节点连接（新增：连接交叉的走廊）
        # 遍历所有走廊节点对，连接距离近的节点（视为走廊交叉点）
...
def plot_path(ax, graph, path):
    ...
    # 绘制完整路径
    ax.plot(x, y, z, color='red', linewidth=3, linestyle='-', marker='o', markersize=5)

    # 标记起点和终点
    ax.scatter(x[0], y[0], z[0], color='green', s=500, marker='*', label='Start', edgecolors='black')
    ax.scatter(x[-1], y[-1], z[-1], color='purple', s=500, marker='*', label='End', edgecolors='black')
    ax.text(x[0], y[0], z[0], f"Start\n{labels[0]}", color='darkgreen', fontweight='bold', fontsize=10)
    ax.text(x[-1], y[-1], z[-1], f"End\n{labels[-1]}", color='darkpurple', fontweight='bold', fontsize=10)
