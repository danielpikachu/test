...
def load_school_data_detailed(filename):
    try:
        ...
        st.error(f"数据文件不存在: {filename}，请检查文件路径是否正确")
        return None
    except json.JSONDecodeError:
        st.error(f"数据文件格式错误，请确保{filename}是有效的JSON文件")
        return None
...
def navigate(graph, start_building, start_classroom, start_level, end_building, end_classroom, end_level):
    ...
    if start_building not in valid_buildings or end_building not in valid_buildings:
        return None, "无效的建筑物选择，仅支持A、B、C栋", None, None
    ...
    try:
        ...
        if start_node not in graph.nodes:
            return None, f"起点教室不存在: {start_building}{start_classroom}@{start_level}", None, None
        if end_node not in graph.nodes:
            return None, f"终点教室不存在: {end_building}{end_classroom}@{end_level}", None, None
...
def main():
    ...
    st.markdown("**💡 可使用地图右侧的控制按钮缩放3D地图**")
    ...
    try:
        ...
        st.success("✅ 校园数据加载成功！初始状态显示A/B/C栋建筑物")
    except Exception as e:
